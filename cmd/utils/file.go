...
func PanicOnError(err error, msg string) {
	if revErr, ok := err.(*Error); (ok && revErr != nil) || (!ok && err != nil) {
		Logger.Panic("Abort: "+msg, zap.Errors("errors", []error{revErr, err}))
	}
}
...
func Empty(dirname string) bool {
	...
	if err != nil {
		Logger.Info("error opening directory", zap.Error(err))
	}
	defer func() {
		_ = dir.Close()
	}()
...
func FindSrcPaths(appImportPath, egretImportPath string, packageResolver func(pkgName string) error) (appSourcePath, egretSourcePath string, err error) {
	var (
	...
	if len(appImportPath) > 0 {
		Logger.Info("Seeking app package", zap.String("app", appImportPath))
	...
	Logger.Info("Seeking remote package", zap.String("using", appImportPath), zap.String("remote", egretImportPath))
	egretPkg, err := build.Default.Import(egretImportPath, appPkgDir, build.FindOnly)
	if err != nil {
		Logger.Info("Resolved called Seeking remote package", zap.String("using", appImportPath), zap.String("remote", egretImportPath))
		packageResolver(egretImportPath)
		egretPkg, err = build.Import(egretImportPath, appPkgDir, build.FindOnly)
		if err != nil {
	...
	egretSourcePath, appSourcePath = egretPkg.Dir[:len(egretPkg.Dir)-len(egretImportPath)], appPkgSrcDir
	return
}
