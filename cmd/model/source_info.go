...
type SourceInfo struct {
	...
	//    e.g. "github.com/kenorld/egret/examples/chat/app/controllers.(*Application).Action"
	...
	// Egret notices files with an init() function and imports that package.
	InitImportPaths []string

	// controllerSpecs lists type info for all structs found under
	// app/controllers/... that embed (directly or indirectly) egret.Controller
...
// e.g. "github.com/kenorld/egret/egret.Controller"
		...
		for i, filteredItem := range filtered {
			if unicode.IsLower([]rune(filteredItem.StructName)[0]) {
				utils.Logger.Info("Debug: Skipping adding spec for unexported type",
					zap.String("type", filteredItem.StructName),
					zap.String("package", filteredItem.ImportPath))
		...
		if spec.PackageName == packageFilter {
			...
			if !found && !strings.HasPrefix(spec.StructName, "Test") {
				utils.Logger.Warn("Type found in package: "+packageFilter+
					...
					zap.String("name", spec.StructName), zap.String("importpath", spec.ImportPath), zap.String("foundstructures", unfoundNames))
			}
		}
	}
	return
...
func (s *SourceInfo) ControllerSpecs() []*TypeInfo {
	if s.controllerSpecs == nil {
		s.controllerSpecs = s.TypesThatEmbed(EgretImportPath+".Controller", "controllers")
	}
...
func (s *SourceInfo) TestSuites() []*TypeInfo {
	if s.testSuites == nil {
		s.testSuites = s.TypesThatEmbed(EgretImportPath+"/testing.TestSuite", "testsuite")
	}
	return s.testSuites
}
