package model_test

import (
	"testing"

	"github.com/kenorld/egret/egret"
	"github.com/stretchr/testify/assert"
)

// Test that the event handler can be attached and it dispatches the event received
func TestEventHandler(t *testing.T) {
	counter := 0
	newListener := func(typeOf egret.Event, value interface{}) (responseOf egret.EventResponse) {
		if typeOf == egret.ENGINE_SHUTDOWN_REQUEST {
			counter++
		}
		return
	}
	// Attach the same handler twice so we expect to see the response twice as well
	egret.AddInitEventHandler(newListener)
	egret.AddInitEventHandler(newListener)
	egret.StopServer(1)
	assert.Equal(t, counter, 2, "Expected event handler to have been called")
}
