...
type (
	// Template the interface that all template engines must implement
	Template interface {
		// ContentType() string
		...
		// assetFunc is a func which returns bytes, use it to load the templates by binary
		// namesFunc returns the template filenames
		LoadAssets(virtualDirectory string, virtualExtension string, assetFunc func(name string) ([]byte, error), namesFunc func() []string) error
	...
	// TemplateFuncs is optional interface for the Template
	...
	// TemplateRawExecutor is optional interface for the Template
	// used to receive and parse a raw template string instead of a filename
	TemplateRawExecutor interface {
